...
### Ver.4.2.20260913
- **서버이용료(결제) 기능 추가**
  - 관리자 사이드바에 `💳 서버이용료` 메뉴 + `/admin/billing.html` 라우팅
  - 로그인 뒤 납부 기한이 **7일 이내이거나 지났을 때만** 안내 팝업 → 결제 화면으로
    (「오늘은 그만 보기」를 누르면 그날은 안 뜬다. 매번 뜨면 성가시다)
  - 결제 방식은 **카드 + 가상계좌** 1회 결제. 정기결제(빌링)는 하지 않는다 —
    나이스페이 빌링은 별도 계약이고, 가상계좌는 원리상 자동이체가 안 된다
  - ⚠️ 결제 열쇠는 이 저장소에 두지 않는다. 승인·기한연장은 호스트의 `site-switch`
    한 곳이 맡고, `server.js` 는 `/api/server-billing*` 을 그리로 **넘기기만** 한다.
    사이트마다 키를 복사해 두면 샐 곳만 늘어나기 때문이다
- 정적 파일 캐시 규칙 수정 — html 이 `public, max-age=0` 으로 새어 나가던 것을 막았다
  (css/js 만 걸러내고 있어서 고친 화면이 안 보이는 일이 있었다)
